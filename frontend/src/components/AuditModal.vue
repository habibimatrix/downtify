<template>
  ...
  >
    <div
      class="bg-base-100 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col"
    >
      <!-- Header -->
      <div
      ...
      >
        <div>
          ...
          <p class="text-xs text-base-content/50 mt-0.5 truncate max-w-sm">
            {{ trackName }}
          </p>
        </div>
      ...
      <div v-else-if="audit" class="flex-1 overflow-y-auto p-4 space-y-2">
        <!-- Spotify-ID banner -->
        <div
          v-if="audit.cache?.spotify_sourced"
          class="flex items-center gap-2 rounded-xl bg-info/10 border border-info/20 px-3 py-2 mb-1"
        >
          <Icon
            icon="clarity:bolt-line"
            class="h-3.5 w-3.5 text-info shrink-0"
          />
          <span class="text-[11px] text-info"
            >Spotify-Download: Artist / Album / Title direkt übernommen ·
            Schritte 1–9 übersprungen</span
          >
        </div>
        <div
        ...
        >
          ...
          >
            ...
            <Icon
              ...
              :class="
                expandedSteps.has(step.step ?? step.name) ? 'rotate-180' : ''
              "
            />
          </button>
          ...
          >
            ...
            <div v-if="step.step === '0.5'" class="space-y-2">
              <div
                v-if="step.action?.includes('vollständiger Skip')"
              ...
              <div v-if="step.hit" class="space-y-1">
                ...
                <div
                  v-if="step.albums?.length"
                  class="flex items-start gap-1.5 text-[11px]"
                >
                  <span class="text-base-content/40 w-12 shrink-0 mt-0.5"
                    >Alben</span
                  >
              ...
              <p
                v-else
                class="text-[11px] text-base-content/40 flex items-center gap-1.5"
              >
            ...
            <!-- Step 0: Tag values -->
            <div
              v-else-if="step.step === 0 && step.values"
              class="grid grid-cols-2 gap-1.5"
            >
              <div
                v-for="(val, key) in step.values"
                :key="key"
                class="flex items-center gap-1.5 text-[11px]"
              >
                <span class="text-base-content/40 capitalize w-12 shrink-0">{{
                  key
                }}</span>
            ...
            <div
              v-else-if="step.step === 1 && step.sources"
              class="overflow-x-auto"
            >
              <table class="w-full text-[11px]">
                <thead>
                  <tr class="text-base-content/40">
                    <th class="text-left py-1 pr-3 font-medium">
                      {{ t('audit.source') }}
                    </th>
                ...
                <tbody>
                  <tr
                    v-for="(meta, src) in step.sources"
                    :key="src"
                    class="border-t border-base-content/5"
                  >
                    <td
                      class="py-1 pr-3 font-medium text-base-content/60 capitalize"
                    >
                      {{ src }}
                    </td>
                    <td class="py-1 px-2 text-base-content/70">
                      {{ meta.genre || '—' }}
                    </td>
                    <td class="py-1 px-2 text-base-content/70">
                      {{ meta.artist || '—' }}
                    </td>
                    <td class="py-1 px-2 text-base-content/70 italic">
                      {{ meta.album || '—' }}
                    </td>
                    <td class="py-1 px-2 text-base-content/70">
                      {{ meta.title || '—' }}
            ...
            <div v-else-if="step.step === 2" class="space-y-2">
              <div
                v-for="(count, field) in step.counts"
                :key="field"
                class="flex items-center gap-2"
              >
                <span
                  class="text-[10px] text-base-content/40 w-12 shrink-0 capitalize"
                  >{{ field }}</span
                >
                <span class="text-xs font-medium text-base-content/80 flex-1">{{
                  count.winner || '—'
                }}</span>
                <span
            ...
            <div
              v-else-if="
                (step.step === 5 || step.step === 7) &&
                (step.written || step.open)
              "
              class="flex gap-3 flex-wrap"
            >
              <div
                v-if="step.written?.length"
                class="flex items-center gap-1.5"
              >
                <Icon
            ...
            <div v-else-if="step.step === 6" class="space-y-1">
              <div
                v-if="step.result && Object.values(step.result).some(Boolean)"
                class="grid grid-cols-2 gap-1.5"
              >
                <div
            ...
            <div
              v-else-if="step.step === 8 || step.step === 9"
              class="grid grid-cols-2 gap-1.5"
            >
              <template
                v-for="(val, key) in step.result || step.ergebnis4 || {}"
                :key="key"
              >
            ...
            <div v-else-if="step.step === 10 && step.org" class="space-y-1.5">
              ...
              >
                ...
                <span
                  v-if="typeof meta === 'object' && meta.source"
                  class="text-[10px] text-base-content/30 italic"
                  >{{ meta.source }}</span
                >
              </div>
              <div
                v-if="step.source"
                class="mt-1 text-[10px] text-base-content/40 italic"
              >
                {{ step.source }}
              </div>
            </div>
            ...
            <div v-else-if="step.step === 11">
              <div v-if="step.rules_applied?.length" class="space-y-1">
                <div
                  v-for="rule in step.rules_applied"
                  :key="rule.field"
                  class="flex items-center gap-2 text-xs"
                >
                  <span class="text-base-content/40 capitalize"
                    >{{ rule.field }}:</span
                  >
                  <span class="text-base-content/60 line-through">{{
                    rule.before
                  }}</span>
                  <Icon
            ...
            <div v-else-if="step.step === '4.5'" class="space-y-1">
              ...
              <div class="flex items-center gap-2 text-[11px]">
                ...
                <span class="text-base-content/30"
                  >({{ Math.round((step.score || 0) * 100) }}%)</span
                >
              </div>
            </div>
            ...
            <div v-else-if="step.step === 12" class="space-y-1.5">
              ...
              >
                ...
                <span class="text-warning"
                  >{{ t('audit.cacheGenreOverwritten') }}:</span
                >
                <span class="text-base-content/50 line-through">{{
                  step.genre_prev
                }}</span>
              ...
              >
                ...
                <span class="text-success"
                  >{{ t('audit.cacheAlbumLearned') }}:</span
                >
                <span class="font-medium">{{ step.album }}</span>
              </div>
              <div
                v-if="!step.genre_overwritten && !step.album_new"
                class="flex items-center gap-1.5 text-[11px]"
              >
                <Icon
                  icon="clarity:check-circle-line"
            ...
            <!-- Cache step (legacy) -->
            <div
              v-else-if="step.name === 'Cache' || step.name === 'Cache-Hit'"
              class="flex items-center gap-2"
            >
              <Icon
                icon="clarity:storage-line"
                class="h-4 w-4"
                :class="step.action === 'hit' ? 'text-success' : 'text-primary'"
              />
              <span class="text-xs">{{
                step.action === 'hit'
                  ? t('audit.cacheHit')
                  : t('audit.cacheWritten')
              }}</span>
              <span
                v-if="step.key"
            ...
            <pre
              v-else
              class="text-[10px] text-base-content/50 whitespace-pre-wrap"
        ...
        <div
          v-if="audit.cache"
          class="rounded-xl border border-base-content/8 px-3 py-2.5 flex items-center gap-3"
        >
          <Icon
            icon="clarity:storage-line"
            class="h-4 w-4 shrink-0"
            :class="
              audit.cache.action?.includes('hit')
                ? 'text-primary'
                : 'text-success'
            "
          />
          <span class="text-xs flex-1">
            ...
            <span v-if="audit.cache.spotify_sourced" class="text-info ml-1"
              >· Spotify-ID</span
            >
          </span>
        </div>
      ...
      <div
        v-else
        class="flex-1 flex flex-col items-center justify-center p-8 gap-2"
      >
        <Icon
...
watch(
  ...
  async (val) => {
    ...
    try {
      ...
      const hasArtistCache = res.data.steps?.some(
        (s) => s.step === '0.5' && s.hit
      )
      const hasFullSkip = res.data.cache?.spotify_sourced
      const autoExpand = hasFullSkip
        ? ['0.5', 12]
        : hasArtistCache
          ? ['0.5', 11, 12]
          : [0, 1, 2, 11]
...
function qualitySummary(ergebnis1, status1) {
  ...
  const done = Object.entries(status1)
    .filter(([, v]) => v === 'abgeschlossen')
    .map(([k]) => k)
  const open = Object.entries(status1)
    .filter(([, v]) => v === 'offen')
    .map(([k]) => k)
  const parts = []
